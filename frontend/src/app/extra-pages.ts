  ...
  template: `
    <main class="admin-content admin-section-content">
      ...
      <form class="settings-grid" [formGroup]="settingsForm" *ngIf="section() === 'settings'" (ngSubmit)="saveSettings()">
        ...
        <section class="settings-card"><h2>Acesso e cadastro</h2><label class="switch-row"><input type="checkbox" formControlName="requireEmailVerification" /> Exigir confirmação de e-mail no cadastro</label><p class="settings-hint">Desligado, quem se cadastra entra direto no aplicativo. Ligado, o sistema envia um código por e-mail (Supabase Auth) e a pessoa só acessa depois de confirmar.</p></section>
...
export class AdminSectionPageComponent implements OnInit {
  ...
  protected readonly settingsForm = this.fb.nonNullable.group({
    ...
    allowRecommendations: true, recommendationApproval: true, allowReviews: true, requireComment: true, professionalSelfRegistration: false, requireEmailVerification: false,
  });
  ngOnInit() {
