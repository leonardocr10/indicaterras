...
export class AdminCrudPageComponent implements OnInit {
  ...
  private readonly configs: Record<AdminResource, { title: string; fields: AdminField[]; columns: string[]; columnKeys: string[] }> = {
    ...
    professionals: { title: 'Profissionais', fields: [{ key: 'name', label: 'Nome' }, { key: 'companyName', label: 'Empresa' }, { key: 'phone', label: 'Telefone', type: 'tel' }, { key: 'whatsapp', label: 'WhatsApp', type: 'tel' }, { key: 'instagram', label: 'Instagram' }, { key: 'city', label: 'Cidade', select: 'city' }, { key: 'neighborhood', label: 'Bairro', select: 'neighborhood' }, { key: 'bio', label: 'Sobre o profissional', type: 'textarea' }, { key: 'approvalStatus', label: 'Aprovação', select: 'options', options: [{ value: 'PENDING', label: 'Pendente' }, { value: 'APPROVED', label: 'Aprovado' }, { value: 'REJECTED', label: 'Recusado' }] }, { key: 'active', label: 'Aparece no aplicativo', type: 'checkbox' }], columns: ['Foto', 'Nome', 'Categoria', 'Cidade', 'WhatsApp', 'Aprovação'], columnKeys: ['avatar', 'name', 'category', 'city', 'whatsapp', 'approvalStatus'] },
